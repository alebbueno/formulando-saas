...
export const EmailTextElement: EmailElementDefinition = {
    ...
    construct: (id) => ({
        id,
        type: 'email-text',
        properties: { text: 'Digite seu texto aqui...', fontSize: 16, color: '#334155', textAlign: 'left', lineHeight: 1.6, paddingTop: 4, paddingBottom: 4 },
    }),
    ...
    toEmailHtml: (element) => {
        ...
        return `<p style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Arial,sans-serif;font-size:${p.fontSize || 16}px;color:${p.color || '#334155'};text-align:${p.textAlign || 'left'};line-height:${p.lineHeight || 1.6};margin:0;padding:${p.paddingTop ?? 4}px 0 ${p.paddingBottom ?? 4}px;">${text}</p>`
    }
}
